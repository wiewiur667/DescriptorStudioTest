...
namespace Caliburn.Micro.Logging
{
	internal class DebugLog : LogBase
	{
		...
		{
			Debug.WriteLine(CreateLogMessage("Exception = {0}", exception), ErrorCategory);
		}
		...
		{
			Debug.WriteLine(CreateLogMessage(format, args), ErrorCategory);
		}
		...
		public override void Error(Exception exception, string format, params object[] args)
		{
			...
			Debug.WriteLine(string.Format(@"{0}{1}Exception = {2}", customMessage, Environment.NewLine, exception), ErrorCategory);
		}
		...
		{
			Debug.WriteLine(CreateLogMessage(format, args), InfoCategory);
		}
		...
		{
			Debug.WriteLine(CreateLogMessage(format, args), WarnCategory);
		}
	}
}